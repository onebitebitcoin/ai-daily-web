...
```bash
python scripts/find_illustration.py "data center servers" --n 5
...
- 이미지를 눈으로 봐라. 카드 10장의 이미지는 URL만 넣고 끝내지 말고 실제로
  ...
  **기사 본문 실사진 → 같은 기업·인물 실사진 → 후보 `image_url` →
  키워드 삽화(`find_illustration.py`, credit 필수) → `media: null`**.
...
아래 다섯 가지를 대조한다. 7.1절의 "본문의 숫자·고유명사를 원문과 대조하라"를
발행 **전** 단계로 끌어온 것이다 — 발행 후에 발견하면 재발행해야 한다.

아래 사례는 자매 프로젝트 `btc-daily-web`의 2026-09-20 발행분을 사후 검증한
결과다. 10장 중 **6장**이 걸렸고, 걸린 유형은 도메인과 무관하게 똑같이 나타난다.
...
실제 사례에서는 **8월 보고서의 상승률**과 **9월 현재가**가 한 문장에 붙어, 최근
닷새 사이에 벌어진 일처럼 읽혔다.

AI 도메인에서 특히 조심할 조합: **벤치마크 점수**(발표 시점의 특정 버전)와 **현재
모델 사양**, **투자 라운드 조달액**과 **기업가치**, **누적 사용자 수**와 **월간
활성 사용자 수**. 3.0.1절의 클러스터링으로 여러 매체를 묶었다면 각 매체가 서로
다른 시점의 숫자를 쓰고 있지 않은지 본다.

#### 둘. 기사 발행일과 사건 발생일을 구분하라

후보의 `published_at`은 **기사가 나온 시각이지 사건이 일어난 시각이 아니다.**
실제 사례에서는 9월 20일 기사가 8월 데이터를 다뤘고, 9월 19일 기사가 **2월에
벌어진 사건**을 다시 짚은 것이었는데, 카드는 어제 일처럼 적었다.

AI 도메인의 전형적인 어긋남:

- **발표일과 출시일이 다르다.** 프리뷰 공개, 웨이트리스트 오픈, 일반 공개가 각각
  다른 날이다. "출시했습니다"라고 쓰기 전에 어느 단계인지 확인한다
- **논문 공개일과 기사일이 다르다.** arXiv 프리프린트가 몇 주 전에 나오고 기사가
  뒤따르는 경우가 흔하다
- **유튜브는 촬영일과 공개일이 다르다.** 설명란의 촬영일 고지를 확인한다. 시황이나
  모델 비교 영상은 그사이 판이 바뀐다
...
| | |
|---|---|
| 나간 문구 | "**블랙록이** ... 커졌다고 **평가했습니다**" |
| 실제 | 블랙록 **ETF 부문 총괄 제이 제이콥스**가 **유튜브 채널에 출연해** 한 말이다. 공식 리서치가 아니고 뒷받침 수치도 기사에 없었다 |
...
AI 소식은 이 구분이 특히 자주 무너진다. 연구원이나 임원의 X 게시물, 팟캐스트 발언,
사내 메모 유출이 "오픈AI가 밝혔다", "구글이 예고했다"로 올라간다. 기관 이름만
주어로 쓰면 카드가 실제보다 무거운 근거를 가진 것처럼 읽힌다.

#### 넷. 원문에 없는 비교급·최상급을 만들지 마라

| | |
|---|---|
| 나간 `quote` | "**2년 만의 최대** 랠리, ..." |
| 실제 | 원문은 "**2019년 이후** 가장 큰 단일 일간 이벤트"라고 적었다. 기간도 대상도 달랐다 |

"사상 최대", "역대 최초", "세계 최초", "처음으로", "N년 만의"는 **원문에 그 표현이
그대로 있을 때만** 쓴다. AI 기사는 "최초의 ~ 모델", "최고 성능"이라는 표현이 홍보
자료에서 넘어오는 경우가 많으니, 그 주장을 **누가 했는지**까지 함께 적는다
(예: `"회사는 ... 최고 성능이라고 밝혔습니다"`).

`quote`는 한 줄이라 임팩트를 노리다 표현을 만들어내기 쉽다. `body`보다 더 엄격하게 본다.

#### 다섯. 수치의 성격과 영문 표현의 뜻을 확인하라

실제 사례에서 한 카드가 두 가지를 동시에 틀렸다. **보유 잔액**을 "매각 규모"로
적었고, 영문 제목의 `eight-year ... test`를 "8년 **만의** 시험"으로 옮겼다. 원문은
"8년에 **걸친** 시험"이라는 뜻이었다.

- **잔액과 유량**, **누적과 연간**, **목표와 실적**, **투자 약정과 집행액**을
  구분한다. 큰 숫자일수록 그 숫자가 무엇의 규모인지 본문에서 확인한다
- AI 기사의 단골 혼동: **기업가치와 조달액**, **데이터센터 투자 계획 총액과 올해
  집행분**, **파라미터 수와 컨텍스트 길이**, **학습 데이터 규모와 모델 크기**
...
반드시 연다. 사후 검증에서 걸린 6장이 전부 이 둘이었다.
